...
%global npm_bare      tsconfig-config
%global sitelib       %{_prefix}/lib/node_modules

Name:           nodejs-sebastienrousseau-tsconfig-config
Version:        0.0.7
Release:        1%{?dist}
Summary:        Shared TypeScript compiler configuration rules adhering to modern 2026 standards

License:        Apache-2.0 OR MIT
URL:            https://github.com/sebastienrousseau/tsconfig-config
Source0:        %{name}-%{version}.tar.gz
BuildArch:      noarch
Requires:       nodejs >= 20

%description
@sebastienrousseau/tsconfig-config is a shareable configuration preset with no runtime
...
install -pm 0644 base.json %{buildroot}%{sitelib}/%{npm_scope}/%{npm_bare}/
install -pm 0644 node.json %{buildroot}%{sitelib}/%{npm_scope}/%{npm_bare}/
install -pm 0644 web.json %{buildroot}%{sitelib}/%{npm_scope}/%{npm_bare}/
