...
<p align="center">
  Shared TypeScript compiler configuration rules adhering to modern 2026 standards
</p>
...
<p align="center">
  ...
  <a href="https://www.npmjs.com/package/@sebastienrousseau/tsconfig-config"><img src="https://img.shields.io/npm/v/%40sebastienrousseau%2Ftsconfig-config.svg?style=for-the-badge&color=fc8d62&logo=npm" alt="npm package" /></a>
  <a href="https://sebastienrousseau.com/tsconfig-config/"><img src="https://img.shields.io/badge/docs-sebastienrousseau.com-brightgreen.svg?style=for-the-badge&logo=github" alt="Documentation" /></a>
...
- [Strict Type Checking & Modern Module Resolution](#strict-type-checking--modern-module-resolution) — before and after rule comparison
...
Install `@sebastienrousseau/tsconfig-config` as a development dependency:
...
TypeScript resolves `extends` through node_modules, so these are used directly.

### tsconfig.json — library

```json
{
  "extends": "@sebastienrousseau/tsconfig-config/base.json",
  "include": ["src"]
}
```

### tsconfig.json — Node application
...
```json
{
  "extends": "@sebastienrousseau/tsconfig-config/web.json",
  "include": ["src"]
}
```
...
`@sebastienrousseau/tsconfig-config` is a focused satellite of the [`@sebastienrousseau/config`](https://github.com/sebastienrousseau/config) suite — a unified ecosystem of 21 single-purpose, zero-overhead developer configurations designed to share a single design philosophy, strict typing, and zero runtime dependencies.
...
`@sebastienrousseau/tsconfig-config` solves this with three deliberate engineering choices:
...
When writing non-typechecked loose JavaScript or legacy TypeScript < 4.5 requiring outdated emit targets like ES3.
